...
class NetworkCustom(Network):
    def from_nx(self, nx_graph, node_size_transf=(lambda x: x), edge_weight_transf=(lambda x: x),
        ...
        edge_weights = [w for (u,v,w) in nx_graph.edges.data('weight', default=0)]
        ...
        if len(edges) > 0:
            for e in edges:
                ...
                if use_weighted_opacity:
                    squashed_w = np.interp(e[2]['weight'], edge_w_range, opacity_range)
                    e[2]["value"] = 1
                    e[2]["color"] = {}
                    e[2]["font"] = {}
...
def visualise_graph_pyvis(graph: nx.DiGraph,
    ...
    interactive_graph = NetworkCustom(height=800, width=800, directed=True, notebook=True)
