...
class CopomuS:
    def __init__(self, opt_arg_file: str = None):
        self.query = ''
        self.target = ''
        self.qidxpos0 = 1
        self.tidxpos0 = 1
        ...
        self.opt_arg_file = opt_arg_file
    ...
    def main(self):
        ...
        bp_func = get_selector(
            self.candidate_selection) if not self.mutation_encoding else get_selector('mutEnc')
        ...
        if not self.mutation_encoding:
            for f in self.candidate_filters:
                func = get_filter(f)
                bp = func(bp, self.query, self.target,
                          self.qidxpos0, self.tidxpos0)

        if not bp:
            sys.stderr.write(
                'There are no base pair candidates left after filtering!\n')
            sys.exit(1)
    ...
    def parse_cmd_args(self):
        ...
        parser.add_argument('--alpha', dest='alpha', type=float,
                            default=defaults['alpha'], help=SUPPRESS)
        parser.add_argument('--beta', dest='beta', type=float,
                            default=defaults['beta'], help=SUPPRESS)

        args = parser.parse_args(args=self.opt_arg_file)
        ...
        # capitalize sequences and replace T -> U
        args.query = args.query.upper().replace('T', 'U')
        args.target = args.target.upper().replace('T', 'U')

        if args.param_file and not os.path.exists(args.param_file):
            sys.stderr.write(
                f'Cannot find provided parameter file: {args.param_file}\n')
            sys.exit(1)
    ...
    def run_tests(self, mutations: List[Mutation]):
        for mut in mutations:
            outcsvcols = list(set(
                [j for i in [mm.outcsvcols for mm in mut.measures] for j in i]))  # combine lists
            outcsvcols = ','.join(outcsvcols)  # join columns
            outcsvcols = 'id1' if not outcsvcols else outcsvcols  # if no col -> select id1

            out_pipes = [j for i in [m.out_pipes for m in mut.measures]
                         for j in i]  # combine lists

            # create args for IntaRNA call
            out_param = [
                f'--out={x}:{os.path.join(gettempdir(), f"CopomuS_{x}")}_{os.getpid()}.temp' for x in out_pipes]

            for mode in ['ww', 'wm', 'mw', 'mm']:  # go through each mutation mode
                # get actual sequences
                q = mut.qw if mode[0] == 'w' else mut.qm
                t = mut.tw if mode[1] == 'w' else mut.tm

                i = IntaRNA()  # call IntaRNA, we have 4 calls per base pair, one for each mutation mode
                data = i.run(q, t, mut.qidxpos0, mut.tidxpos0, outcsvcols,
                             self.threads, 1, self.param_file, out_param)
    ...
    def output_csv(self, mutations: List[Mutation]):
        ...
        # WRITE CSV
        headers = [j for i in
                   [[f'{m.id}_rank'] + [f'{m.id}_{k}' for k in m.score.keys()]
                    for m in mutations[0].measures]
                   for j in i]
        writer.writerow(['mutation', 'rank', 'qIndex', 'tIndex',
                        'bpWildtype', 'bpMutated'] + headers)
        ...
        for m in mutations:
            ...
            writer.writerow([str(m), m.ranks['total'], m.query_n,
                            m.target_n, m.bp_ww, m.bp_mm] + values)  # write csv
