...
infom = info.melt(good_cols, value_vars=get_true_interaction_cols(
    info, 'energies'), var_name='energies_idx', value_name='energies')
...
mutation_logm = mutation_log.melt(mutation_cols, value_vars=get_true_interaction_cols(
    mutation_log, 'energies'), var_name='energies_idx', value_name='energies')

for k in ['binding_sites', 'binding_rates_dissociation', 'eqconstants']:
    dfm = info.melt(good_cols, value_vars=get_true_interaction_cols(
        info, k), var_name=f'{k}_idx', value_name=k)
    infom[k] = dfm[k]
    dfm = mutation_log.melt(mutation_cols, value_vars=get_true_interaction_cols(
        mutation_log, k), var_name=f'{k}_idx', value_name=k)
...
for k in ['binding_rates_dissociation', 'eqconstants', 'energies']:
    infom[f'{k}_diffs'] = info.groupby(['circuit_name'])[get_true_interaction_cols(info, f'{k}')].apply(
        lambda x: x - x.iloc[0]).melt(value_vars=get_true_interaction_cols(info, f'{k}'), var_name='idx', value_name=f'{k}_diffs')[f'{k}_diffs']
    mutation_logm[f'{k}_diffs'] = mutation_log.groupby(['circuit_name'])[get_true_interaction_cols(mutation_log, f'{k}')].apply(
        lambda x: x - x.iloc[0]).melt(value_vars=get_true_interaction_cols(mutation_log, f'{k}'), var_name='idx', value_name=f'{k}_diffs')[f'{k}_diffs']
...
for c in relevant_cols:
    ...
    named_aggs.update({c + '_std_normed_by_mean': pd.NamedAgg(column=c,
...
info_summ = mutation_logm.groupby(
    ['circuit_name', 'mutation_num', 'sample_name'], as_index=False).agg(**named_aggs)
...
info_e[(info_e[isb] == True) & (info_e['mutation_num'] == 0)
       ][['mutation_positions', isb, r]]
...
sns.jointplot(infom[infom['mutation_num'] > 0],
              x='frac_muts_in_binding_site', y='energies', hue='mutation_num')
